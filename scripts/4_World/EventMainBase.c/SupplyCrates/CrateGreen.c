class BattlegroundsGreenLoadout extends BattlegroundsLoadoutCrate1
{
    override void EEItemAttached(EntityAI item, string slot_name)
	{
        super.EEItemAttached(item, slot_name);
        if (item.IsKindOf("BlueTeamFlagItem") || item.IsKindOf("RedTeamFlagItem") || item.IsKindOf("OrangeTeamFlagItem"))
        {
            ReplenishBaseSupplies();
            AddCaptureRewards();
            SendFlagClaimedRPC(item, "greenteamclaim");
        }
    }
    ...
    void SendFlagClaimedRPC(EntityAI item, string action)
    {
        ...
        {   
            ...
            // 'action' specifies which team claimed it (e.g., "greenteamclaim")
    ...
    override void ReplenishBaseSupplies()
    {
        ClearInventory();
        ClearAttachments();
        array<string> itemsCargo = {"Hatchet", "NailBox", "GreenTeamLock", "LargeTent"};
